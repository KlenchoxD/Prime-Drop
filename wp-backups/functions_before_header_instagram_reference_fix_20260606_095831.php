...
add_action('wp_footer', function() {
    ...
    @media (min-width: 1000px) {
      ...
      #header [data-device="desktop"] [data-id="account"] {
        display: none !important;
      }
    ...
    (function() {
      function addReferenceMenuItems() {
        var menu = document.querySelector('#header [data-device="desktop"] .header-menu-1 .menu, #header [data-device="desktop"] .header-menu-1 > ul');
        if (!menu) return;
        var bolsos = menu.querySelector('li.pd-bolsos-menu-parent');
        if (!bolsos) {
          var links = menu.querySelectorAll(':scope > li > a');
          links.forEach(function(link) {
            if ((link.textContent || '').trim().toUpperCase() === 'BOLSOS') {
              bolsos = link.closest('li');
            }
          });
        }
        if (!bolsos || menu.querySelector('.pd-reference-menu-mujer')) return;

        function item(label, href, cls) {
          var li = document.createElement('li');
          li.className = 'menu-item menu-item-type-custom menu-item-object-custom ' + cls;
          var a = document.createElement('a');
          a.className = 'ct-menu-link';
          a.href = href;
          a.textContent = label;
          li.appendChild(a);
          return li;
        ...
        menu.insertBefore(item('MUJER', '/product-category/mujer/', 'pd-reference-menu-mujer'), bolsos);
        menu.insertBefore(item('HOMBRE', '/product-category/hombre/', 'pd-reference-menu-hombre'), bolsos);
      }

      function run() {
        addReferenceMenuItems();
