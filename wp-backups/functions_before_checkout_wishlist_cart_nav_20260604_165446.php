...
add_action('wp_footer', function() {
    ...
    (function() {
      ...
      function fixCheckoutLabels() {
        var account = document.querySelector('label[for="createaccount"]');
        if (account) account.textContent = 'Crear una cuenta para guardar tus pedidos (opcional)';

        var ship = document.querySelector('label[for="ship-to-different-address-checkbox"] span, label[for="ship-to-different-address-checkbox"]');
        if (ship) ship.textContent = 'Enviar a otra direccion (opcional)';
      ...
      function cleanWishlist() {
        ...
        if (!/vac[ií]a|no se ha añadido|no products|wishlist/i.test(text)) return;
