...
add_action('wp_head', function() {
    ...
    (function() {
      ...
      if (window.MutationObserver) {
        new MutationObserver(replaceBagIconEarly).observe(document.documentElement, { childList: true, subtree: true });
      }
    })();
    </script>
    <?php
...
add_action('wp_footer', function() {
    ...
    (function() {
      ...
      function fixCheckoutLabels() {
        ...
        if (account) account.innerHTML = 'Crear cuenta para guardar tus pedidos <span class="pd-optional">(opcional)</span>';

        var shipInput = document.querySelector('#ship-to-different-address-checkbox, input[name="ship_to_different_address"]');
        var ship = document.querySelector('label[for="ship-to-different-address-checkbox"] span') || document.querySelector('label[for="ship-to-different-address-checkbox"]') || shipInput?.closest('label') || shipInput?.parentElement?.querySelector('label');
        if (ship) ship.innerHTML = 'Enviar a otra direccion <span class="pd-optional">(opcional)</span>';
